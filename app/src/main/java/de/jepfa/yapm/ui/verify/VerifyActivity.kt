...
class VerifyActivity : NfcBaseActivity() {
    ...
    private fun handleScannedData(scanned: String) {
        ...
        if (scanned.startsWith(Encrypted.TYPE_ENC_MASTER_KEY)) {
            ...
            if (masterPassword != null && scannedMasterPassword.isValid() && scannedMasterPassword.isEqual(masterPassword)) {
            ...
            val encMasterPasswordTokenKey = PreferenceService.getEncrypted(PreferenceService.DATA_MASTER_PASSWORD_TOKEN_KEY, this)
            encMasterPasswordTokenKey?.let {
                ...
                val masterPassword = getMasterPasswordFromSession()
                if (masterPassword != null && encMasterPasswordFromScanned.isValid() && encMasterPasswordFromScanned.isEqual(masterPassword)) {
