...
![Downloads last month](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fgithub.com%2Fkitswas%2Ffdroid-metrics-dashboard%2Fraw%2Frefs%2Fheads%2Fmain%2Fprocessed%2Fmonthly%2Fde.jepfa.yapm.json&query=%24.total_downloads&logo=fdroid&label=Downloads%20last%20month)

![Downloads (all time)](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fgithub.com%2Fkitswas%2Ffdroid-metrics-dashboard%2Fraw%2Frefs%2Fheads%2Fmain%2Fprocessed%2Ftotal%2Fde.jepfa.yapm.json&query=%24.total_downloads&logo=fdroid&label=Downloads%20(all%20time))

# ANOTHERpass

This Android app is another way to manage passwords. It addresses the problem of having to remember a strong master password, which is often a weak one because it is created by humans.

To avoid a weak master password, the app generates it, not the user! This ensures a strong and unpredictable master password.

The app requires two factors to unlock the vault, the generated master password (ownership in the sense of a key) and a PIN (knowledge, in the sense of a remembered secret).

One consistent idea of the app is to use QR codes or NFC tags as offline-storage for credentials and secrets, such as the master password. Printed QR codes and NFC tags can be easily stored at a protected analogue place. With this and the ability to separate the encrypted vault and the encryption key, it is possible to implement your own specific security and backup strategy.
...
The app ships dozens of useful features to make it easy *and* secure to manage passwords in a self-responsive way.
